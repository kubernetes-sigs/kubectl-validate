...
func TestValidatorFactory_TestPatcher(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			client: openapiclient.NewLocalCRDFiles(os.DirFS("./testdata/crds/")),
			want: &unstructured.Unstructured{
				Object: map[string]interface{}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			factory, err := New(tt.client, []string{"IgnoredKind", "some.group/version/IgnoredGVK"})
			assert.NoError(t, err)
			assert.NotNil(t, factory)
			assert.Equal(t, factory.ignoredKinds, []string{"IgnoredKind", "some.group/version/IgnoredGVK"})
